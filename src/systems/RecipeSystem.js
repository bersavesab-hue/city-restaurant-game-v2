...
function validateRecipe(recipe) {
  ...
  if (!dishCatalogSystem.exists(recipe.dishId)) {
    throw new Error(
  ...
  for (const item of recipe.ingredients) {
    if (
      ...
      !ingredientCatalogSystem.exists(item.ingredientId)
    ) {
      throw new Error(
...
class RecipeSystem {
  load(records, { overwrite = false } = {}) {
    if (!Array.isArray(records)) {
      throw new TypeError("Recipe records must be an array");
    }

    records.forEach(validateRecipe);
